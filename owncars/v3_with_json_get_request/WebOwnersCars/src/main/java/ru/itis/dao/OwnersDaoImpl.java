...
public class OwnersDaoImpl implements OwnersDao {
    ...
    private final String SQL_ALL_OWNER_OF_AGE = "SELECT * FROM owners WHERE age = ?";
    ...
    public Owner findLogin(String login) {
        try {
            ...
            result.next();
            return new Owner(result.getInt("owner_id"),result.getString("city"),result.getInt("age"),result.getString("name"),result.getString("login"),result.getString("password"));
